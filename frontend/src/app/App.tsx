...
function ShopModal({ char, onBuy, onClose }: { char: Character; onBuy: (item: Item) => void; onClose: () => void }) {
  ...
  const [npcFace, setNpcFace]         = useState<NpcFace>("talk"); // start talking initially but invisible
  ...
  useEffect(() => {
    ...
    // 2. Shop panel slides in
    after(480, () => setPanelVisible(true));
    // 3. Show greeting bubble with npc_01 (talking)
    after(800, () => {
      setNpcFace("talk");
      setBubbleText("Welcome, adventurer! Looking for something special today?");
      setBubbleShow(true);
    });
    // 4. Bubble fades out, switch to idle (npc_03)
  ...
  useEffect(() => {
    if (npcFace === "idle") {
      const waitTime = 6000 + Math.random() * 8000; // random delay 6-14 seconds
      const t = setTimeout(() => {
        setNpcFace("talk");
        setBubbleText(IDLE_QUOTES[Math.floor(Math.random() * IDLE_QUOTES.length)]);
        setBubbleShow(true);

        after(5000, () => setBubbleShow(false)); // Was 3500 (+1500)
        after(5500, () => { setNpcFace("idle"); setBubbleText(""); }); // Was 4000 (+1500)
      }, waitTime);
  ...
  function handleBuy(item: Item) {
    ...
    timersRef.current = [];
    // Show happy face + bounce + random quote
    ...
    // Return to idle after 4.7s (Was 3.2s)
    after(4700, () => setBubbleShow(false));
    after(5200, () => { setNpcFace("idle"); setBubbleText(""); }); // Was 3.7s
  }

  function handleClose() {
    // 1. NPC says farewell with talk face
    timersRef.current.forEach(clearTimeout);
    timersRef.current = [];
    setNpcFace("talk");
    setBubbleText("Come back soon, adventurer!");
    setBubbleShow(true);
    // 2. Shop panel slides out
  ...
  return (
    <div style={{
      ...
      <style>{`
        @keyframes shop-npc-in {
          0%   { transform: translateX(-115%); }
          72%  { transform: translateX(6px); }
          100% { transform: translateX(0); }
        }
        @keyframes shop-npc-out {
        ...
        @keyframes shop-panel-out {
          0%   { transform: translateX(0); opacity: 1; }
          100% { transform: translateX(80px); opacity: 0; }
        }
        @keyframes shop-item-in {
          0%   { transform: translateX(36px) scaleX(0.92); opacity: 0; }
          65%  { transform: translateX(-2px) scaleX(1.01); opacity: 1; }
        ...
        @keyframes npc-bounce-once {
          0%   { transform: translateX(-50%) translateY(0px) scale(1); }
          25%  { transform: translateX(-50%) translateY(-22px) scale(1.04); }
          55%  { transform: translateX(-50%) translateY(6px) scale(0.97); }
      ...
      <div style={{
        ...
        animation: npcExiting
          ? "shop-npc-out 0.7s cubic-bezier(0.4, 0, 0.8, 0.4) forwards"
          : npcEntered
          ? "shop-npc-in 0.75s cubic-bezier(0.34, 1.4, 0.64, 1) forwards"
          : "none",
        ...
        {/* NPC image — full body, natural aspect ratio */}
        <img
          src={npcSrc}
          alt="Shop NPC"
          style={{
            ...
            display: "block",
            animation: npcBounce
              ? "npc-bounce-once 0.6s cubic-bezier(0.34, 1.5, 0.64, 1) forwards"
              : "npc-breathe 3.8s ease-in-out 0.5s infinite",
          }}
        />
        ...
        {bubbleText && (
          <div style={{
            ...
            animation: bubbleShow
              ? "bubble-pop-in 0.4s cubic-bezier(0.34,1.5,0.64,1) forwards"
              : "bubble-fade-out 0.5s ease forwards",
            pointerEvents: "none",
          }}>
            <div style={{
              background: cream,
              border: `3px solid ${wood3}`,
              borderRadius: 12,
              padding: "10px 12px",
              fontFamily: NU,
              fontSize: 12,
              color: wood1,
              lineHeight: 1.5,
              boxShadow: `0 4px 16px rgba(100,50,0,0.35)`,
              position: "relative",
            }}>
              {bubbleText}
              {/* Bubble tail — pointing down */}
              <div style={{
                position: "absolute", bottom: -10, left: "50%", marginLeft: -7,
                width: 0, height: 0,
                borderLeft: "7px solid transparent",
                borderRight: "7px solid transparent",
                borderTop: `10px solid ${wood3}`,
              }} />
              <div style={{
        ...
        {/* NPC name plate — overlaps character at bottom */}
        <div style={{
          ...
          <div style={{ fontFamily: PX, fontSize: 9, color: cream, letterSpacing: 2, textShadow: `0 1px 3px ${wood1}` }}>
            MIRA
          </div>
          <div style={{ fontFamily: NU, fontSize: 10, color: wood5, marginTop: 2 }}>
            General Shopkeeper
          </div>
        </div>
      </div>

      {/* ── RIGHT PANEL: Shop (3/4 screen) ── */}
      <div style={{
        flex: 1,
        display: "flex",
        ...
        animation: panelExiting
          ? "shop-panel-out 0.6s cubic-bezier(0.4, 0, 0.8, 0.4) forwards"
          : panelVisible
          ? "shop-panel-in 0.65s cubic-bezier(0.34, 1.2, 0.64, 1) forwards"
          : "none",
        ...
        <div style={{
          ...
          borderBottom: `4px solid ${wood1}`,
          boxShadow: `0 4px 12px rgba(0,0,0,0.4)`,
        }}>
          <div style={{
            height: 8,
            background: `repeating-linear-gradient(90deg, ${wood3} 0px, ${wood3} 12px, ${wood4} 12px, ${wood4} 24px)`,
          }} />
          <div style={{ padding: "12px 20px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontFamily: PX, fontSize: 12, color: cream, letterSpacing: 2, textShadow: `0 2px 4px ${wood1}` }}>
                🏪 NEKO GENERAL STORE
              </div>
              <div style={{ fontFamily: NU, fontSize: 11, color: wood5, marginTop: 3 }}>
                Fine goods for fine adventurers ☕
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <button onClick={handleClose} style={{
                ...
                border: `2px solid ${wood1}`,
                cursor: "pointer",
                color: cream, width: 32, height: 32,
                display: "flex", alignItems: "center", justifyContent: "center",
                boxShadow: `inset 0 1px 0 ${wood4}60, 0 2px 4px rgba(0,0,0,0.4)`,
        ...
        <div style={{
          ...
          {(["weapon", "armor", "consumable", "accessory"] as const).map(t => (
            <button key={t} onClick={() => handleTabChange(t)} style={{
              ...
              border: "none",
              borderRight: `1px solid ${wood3}50`,
              ...
              <span style={{ fontFamily: PX, fontSize: 7, letterSpacing: 0.5, color: tab === t ? tabActive : wood2 }}>
                {tabLabels[t]}
              </span>
            </button>
          ))}
        ...
        <div style={{
          ...
          {items.map((item, idx) => (
            <div key={`${tabKey}-${item.id}`} style={{
              ...
              <div style={{
                ...
                background: `linear-gradient(90deg, ${wood3}, ${wood4}, ${wood3})`,
              }} />
              <div style={{ paddingTop: 4 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                  <span style={{ fontFamily: PX, fontSize: 9, color: wood1 }}>{item.name}</span>
                  {item.damage && (
                    <span style={{ fontFamily: MO, fontSize: 9, color: "#7a4010", padding: "1px 6px", background: `${wood5}80`, border: `1px solid ${wood4}` }}>
                      {item.damage}
                    </span>
                  )}
                  {item.ac && (
                    <span style={{ fontFamily: MO, fontSize: 9, color: "#305080", padding: "1px 6px", background: "#d8e8f8", border: "1px solid #8ab0d0" }}>
                      AC {item.ac}
                    </span>
                  )}
                </div>
                <div style={{ fontFamily: NU, fontSize: 11, color: "#7a5030", lineHeight: 1.4 }}>
                  {item.description}
                </div>
              </div>
              <button
                onClick={() => handleBuy(item)}
                disabled={char.gold < item.value}
                style={{
          ...
          {items.length === 0 && (
            <div style={{ gridColumn: "1 / -1", textAlign: "center", color: wood3, fontFamily: NU, fontSize: 14, padding: 48 }}>
        ...
        <div style={{
          ...
          padding: "10px 20px",
          borderTop: `3px solid ${wood3}`,
          background: `linear-gradient(180deg, ${wood4}40 0%, ${wood3}30 100%)`,
          display: "flex", justifyContent: "center",
        }}>
          <span style={{ fontFamily: PX, fontSize: 7, color: wood2, letterSpacing: 2 }}>
            ⚒ NEKO GOODS · QUALITY GUARANTEED ⚒
          </span>
        </div>
      </div>
    </div>
